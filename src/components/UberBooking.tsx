...
const UberBooking: React.FC<UberBookingProps> = ({ destination, onRideBooked }) => {
  ...
  const handleBookRide = async (rideType: string) => {
    ...
    try {
      ...
      console.log('Booking Uber ride with request:', request);
      const result = await uberService.bookUberRide(request);
      console.log('Uber booking result:', result);
          ...
          <div className="space-y-3">
            ...
            <div className="text-center">
              ...
              <Button
                ...
                size="sm"
                onClick={() => {
                  const webLink = `https://m.uber.com/ul/?action=setPickup&pickup[latitude]=${userLocation?.latitude}&pickup[longitude]=${userLocation?.longitude}&pickup[nickname]=UW%20Husky%20Union&dropoff[latitude]=${destination.latitude}&dropoff[longitude]=${destination.longitude}&dropoff[nickname]=${encodeURIComponent(destination.name)}`;
